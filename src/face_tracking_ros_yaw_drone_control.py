...
def goodFeatures(img,x,y):
    ...
    img_height = int(img.shape[0]*.1)
    img_width = int(img.shape[1]*.1)
...
def image_callback(ros_image):
	...
	if FACE_TRACKING ==1:
		...
		mean_x,mean_y = int(cx/i),int(cy/i)
		   
		xo,yo = mean_x,mean_y
		cv2.circle(frame_tracking,(mean_x,mean_y), 6, (20,20,20,100), -1)  # Drawing circle at the centre of the frame
		cv2.imshow("Tracked Features",frame_tracking)
		cv2.waitKey(5)
	...
	if fw > 0:
		...
		while not rospy.is_shutdown():
			...
			if in_air == 1:
				...
				pubCommand.publish(Twist(Vector3(0,0,0.3),Vector3(0,0,0)))
			...
			#twist.angular.y = 0
			#twist.angular.z = ex
			pubCommand.publish(Twist(Vector3(0,0,0),Vector3(0,0,ex)))
			rospy.sleep(2.0)
